...
void SubBytes(unsigned char ptr_stateTab[4][4]) {
	for (int i = 0; i <= 3; i++) {
		for (int j = 0; j <= 3; j++) {
			ptr_stateTab[i][j] = S_Box[ptr_stateTab[i][j] >> 4][(ptr_stateTab[i][j] & 0x0F)];
		}
	}
}
...
void InvSubBytes(unsigned char ptr_stateTab[4][4]) {
	for (int i = 0; i <= 3; i++) {
		for (int j = 0; j <= 3; j++) {
			ptr_stateTab[i][j] = rsbox[ptr_stateTab[i][j] >> 4][(ptr_stateTab[i][j] & 0x0F)];
		}
	}
}
